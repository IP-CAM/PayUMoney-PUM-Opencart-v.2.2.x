<?php /* Opencart Module v2.2 for PayUMoney and Citrus Payment Gateway - Copyrighted file - Please do not modify/refactor/disasseble/extract any or all part content  */ ?>
<?php 
...
class ControllerPaymentPumcp extends Controller 
{
	public function index()
	{
		...
		if($order_info['payment_method'] == 'PayUMoney') {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'payment/pumcp_payu.tpl')) {
				return $this->load->view($this->config->get('config_template') . 'payment/pumcp_payu.tpl', $data);	
			} else {
				return $this->load->view('payment/pumcp_payu.tpl', $data);
			}		
		}
		
		if($order_info['payment_method'] == 'CitrusPay') {
			$data=$this->process_citrus();
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'payment/pumcp_citrus.tpl')) 
			{
				return $this->load->view($this->config->get('config_template') . 'payment/pumcp_citrus.tpl',$data);	
			} 
			else 
			{
				return $this->load->view('payment/pumcp_citrus.tpl',$data);
			}	
		}
	}
	...
	private function process_citrus()
	{
		...
		$data['redir_url'] = $this->url->link('payment/pumcp/callback_citrus', '', 'SSL');			
		$data['notify_url'] = $this->url->link('payment/pumcp/callback_citrus', '', 'SSL');
	...
	private function process_payu() {	
        ...
        $data['country']=$order_info['payment_country'];
		$data['Pg'] = 'CC';
		$data['surl'] = $this->url->link('payment/pumcp/callback_payu');//HTTP_SERVER.'/index.php?route=payment/payu/callback';
		$data['Furl'] = $this->url->link('payment/pumcp/callback_payu');//HTTP_SERVER.'/index.php?route=payment/payu/callback';
      ...
      //$this->data['furl'] = $this->url->link('checkout/cart');//HTTP_SERVER.'/index.php?route=payment/payu/callback';
		$data['curl'] = $this->url->link('payment/pumcp/callback_payu');
			 ...
			 if (isset($this->request->post['status']) && $this->request->post['status'] == 'success') {
				...
				foreach($this->request->post as $k => $val){
					...
					if($sentHashString==$this->request->post['hash']){
							...
							if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'payment/pumcp_success.tpl')) {
								$this->response->setOutput($this->load->view($this->config->get('config_template') . 'payment/pumcp_success.tpl', $data));
							} else {
								$this->response->setOutput($this->load->view('payment/pumcp_success.tpl', $data));
							}									
					}			 
		        ...
		        if(isset($this->request->post['status']) && $this->request->post['unmappedstatus'] == 'userCancelled')
				{
				 ...
				 if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'payment/pumcp_cancelled.tpl')) {
					$this->response->setOutput($this->load->view($this->config->get('config_template') . 'payment/pumcp_cancelled.tpl', $data));
				} else {
				    $this->response->setOutput($this->load->view('payment/pumcp_cancelled.tpl', $data));
				}
				}
				else {
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'payment/pumcp_failure.tpl')) {
					$this->response->setOutput($this->load->view($this->config->get('config_template') . 'payment/pumcp_failure.tpl', $data));
				} else {
					$this->response->setOutput($this->load->view('payment/pumcp_failure.tpl', $data));
				}	
